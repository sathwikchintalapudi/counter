...
import com.everon.sessioninfo.functionalinterface.TriPredicate;
...
import static com.everon.sessioninfo.domain.SessionStoreConstants.TOTAL_MINUTES_IN_HOUR;
import static com.everon.sessioninfo.domain.SessionStoreConstants.TOTAL_SECONDS_IN_DAY;
import static com.everon.sessioninfo.domain.SessionStoreConstants.TOTAL_SECONDS_IN_MINUTE;
...
public class HitCounter {
    ...
    private final int[] timeRecorder = new int[TOTAL_SECONDS_IN_MINUTE];

    private final int[] hitRecorder = new int[TOTAL_SECONDS_IN_MINUTE];
    ...
    public void hit(LocalDateTime dateTime) {
        try {
            ...
            int index = timeStamp % TOTAL_SECONDS_IN_MINUTE;
            writeLock.lock();
            try {
    ...
    public int getHits(LocalDateTime dateTime) {
        ...
        TriPredicate triPredicate = (upperLimit > lowerLimit) ?
                ...
                (timeStampInfo, upperLimitInfo, lowerLimitInfo) -> (timeStampInfo >= lowerLimitInfo && timeStampInfo <= TOTAL_SECONDS_IN_DAY || timeStampInfo >= 0 && timeStampInfo <= upperLimitInfo);
        int count = 0;
    ...
    private int getLowerLimit(int timeStamp) {
        int lowerLimit = timeStamp - TOTAL_SECONDS_IN_MINUTE;
        if (lowerLimit < 0) {
            return TOTAL_SECONDS_IN_DAY + lowerLimit;
        }
        return lowerLimit;
    }

    private int getTimeStamp(LocalDateTime dateTime) {
        return dateTime.getHour() * TOTAL_MINUTES_IN_HOUR * TOTAL_SECONDS_IN_MINUTE + dateTime.getMinute() * TOTAL_SECONDS_IN_MINUTE + dateTime.getSecond();
    }
}
